...
class SyncWeiwenjiaCustomers extends Command
{
    ...
    public function handle(): int
    {
        ...
        $login = Http::timeout(30)->post($base . '/api/v2/auth/login', [
            'device' => 'open_api',
            'login' => env('WEIWENJIA_CRM_LOGIN'),
            'password' => env('WEIWENJIA_CRM_PASSWORD'),
        ]);
        ...
        $configuredOwnerId = (int) env('WEIWENJIA_CRM_OWNER_USER_ID', 0);
        $ownerId = ($configuredOwnerId && User::whereKey($configuredOwnerId)->exists())
            ? $configuredOwnerId
            : User::whereHas('roles', fn($q) => $q->where('alias', 'admin'))->value('id');
        $ownerId = $ownerId ?: User::query()->value('id');
        if (! $ownerId) {
            $this->error('未配置 WEIWENJIA_CRM_OWNER_USER_ID，无法创建客户');
            return self::FAILURE;
        }
        ...
        do {
            ...
            if (! $response->successful()) {
                $this->error('客户列表请求失败，页码：' . $page);
                return self::FAILURE;
            }
            $customers = data_get($response->json(), 'data.customers', []);
            foreach ($customers as $item) {
                $this->syncCustomer($item, $ownerId);
                $total++;
            }
            $page++;
            $hasMore = count($customers) >= $size;
        } while ($hasMore);
    ...
    private function syncCustomer(array $item, int $ownerId): void
    {
        ...
        if ($phone || $tel || ! $customer->contacts()->exists()) {
            $contact = $customer->contacts()->firstOrNew(['is_primary' => true]);
            $contact->fill(['contact_name' => $contactName, 'mobile' => $phone ?: $tel, 'wechat_no' => $address['wechat'] ?? null, 'is_primary' => true]);
            $contact->save();
        }
    }
}
